...
#include "../event/EntityPreDestroyedEvent.h"
...
void EntityManager::destroy(const Entity& entity)
{
	...
	Event::transmit<EntityPreDestroyedEvent>(entity);
...
Entity EntityManager::create(const std::string& name)
{
	...
	Entity entity(id, m_versions[id], this);
	Event::transmit<EntityCreatedEvent>(entity);
