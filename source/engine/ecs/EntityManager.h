...
public:
	...
	template <class C>
    const ComponentPtr<C> getComponent() const;
	...
	uint64_t getUID() const { return static_cast<uint64_t>(m_id) | (static_cast<uint64_t>(m_version) << 32); }

    EntityID getID() const { return m_id; }

    EntityVersion getVersion() const { return m_version; }

	bool operator <(const Entity& other) const { return getUID() < other.getUID(); }
private:
    template <class C>
    C* getComponentPtr();
...
void EntityManager::addComponent(const Entity& entity, Args&&... args)
{
	...
	Event::transmit<ComponentAddedEvent<C>>(entity);
}
...
template <class C>
const ComponentPtr<C> Entity::getComponent() const
{
	assert(valid() && "The entity is invalid.");
	return m_manager->getComponent<C>(*this);
}
