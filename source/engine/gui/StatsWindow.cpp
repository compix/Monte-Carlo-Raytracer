...
void StatsWindow::update()
{
	...
	auto elapsedTimeCPUInfo = QueryManager::getHierarchicalElapsedTimeInfo(QueryTarget::CPU);
	auto elapsedTimeGPUInfo = QueryManager::getHierarchicalElapsedTimeInfo(QueryTarget::GPU);
...
void StatsWindow::onElapsedTimeInfoItem(const Hierachical<ElapsedTimeInfoBag>& timeInfo, QueryTarget target)
{
    auto name = timeInfo.data[ElapsedTimeInfoType::PER_FRAME].getName();
    ...
    if (ImGui::TreeNode(name.c_str()))
    {
        ...
        std::stringstream ss;
        ss << "Average in the last second: " << timeInfo.data[selectedType].getAverageInMicroseconds() / 1000.0 << " ms";
        ImGui::Text(ss.str().c_str());
        ss.str("");
        ss.clear();
        ss << "Highest in the last second: " << timeInfo.data[selectedType].getMaxInMicroseconds() / 1000.0 << " ms";
        ImGui::Text(ss.str().c_str());

        plotHistogram(timeInfo.data[selectedType], guiData);

		for(auto& child : timeInfo.children)
			onElapsedTimeInfoItem(child, target);
