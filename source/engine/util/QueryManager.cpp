...
struct InternalElapsedTimeInfo
{
    ...
    // Only begin0 [code...] begin1 [code...] end1 [code...] end0 with an arbitrary amount of begin/end calls in between.
...
std::vector<Hierachical<ElapsedTimeInfoBag>> QueryManager::getHierarchicalElapsedTimeInfo(QueryTarget target)
{
	ElapsedTimeMap* entryMap = getElapsedTimeMap(target);

	std::vector<Hierachical<ElapsedTimeInfoBag>> info;

	std::function<void(Hierachical<ElapsedTimeInfoBag>* parent, InternalElapsedTimeInfo* info)> updateHierarchy;

	updateHierarchy = [&updateHierarchy](Hierachical<ElapsedTimeInfoBag>* parent, InternalElapsedTimeInfo* info) {
		for (auto child : info->children)
		{
			Hierachical<ElapsedTimeInfoBag> c(child->info);
			updateHierarchy(&c, child);
			c.setParent(parent);
		}
	};
	...
	for (auto& p : *entryMap)
	{
		// Add new entry if it doesn't have a parent, if it does then it was already handled
		if (!p.second->parent)
		{
			info.push_back(Hierachical<ElapsedTimeInfoBag>(p.second->info));
			Hierachical<ElapsedTimeInfoBag>& entry = info[info.size() - 1];

			// Fill hierarchy
			updateHierarchy(&entry, p.second.get());
		}
	}
