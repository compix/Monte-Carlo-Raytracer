...
namespace math
{
    ...
    */
    ...
    inline glm::vec3 polarToVector(float radius, float elevation, float azimuth)
    {
		...
		return glm::vec3(radius * sinf(elevation) * cosf(azimuth),
			radius * cosf(elevation),
			radius * sinf(elevation) * sinf(azimuth));
    }

    inline Polar vectorToPolar(glm::vec3 v)
    {
        float r = sqrtf(v.x * v.x + v.y * v.y + v.z * v.z);
        return Polar(r, glm::acos(v.y / r), glm::atan(v.z, v.x));
    }
}
