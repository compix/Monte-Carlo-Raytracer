...
bool Ray::intersects(const BBox& aabb, glm::vec2& t) const
{
    ...
    t.x = glm::max(glm::max(t1.x, t1.y), t1.z); // near hit
    t.y = glm::min(glm::min(t2.x, t2.y), t2.z); // far hit

    return t.x <= t.y;
}

bool Ray::intersectsCapsule(const glm::vec3& p, const glm::vec3& q, float r, float& t)
{
    float constexpr maxDistance = 100.0f;
    if (intersection::segmentCapsule(origin, origin + direction * maxDistance, p, q, r, t))
    {
        t *= maxDistance;
        return true;
    }
...
bool Ray::intersectsSphere(const glm::vec3& pos, float radius, float& t)
{
    glm::vec3 m = origin - pos;
    float a = glm::dot(direction, direction);
    float b = glm::dot(direction, m);
...
bool Ray::intersectsCylinder(const glm::vec3& p, const glm::vec3& q, float r, float& t, float maxDistance)
{
    if (intersection::segmentCylinder(origin, origin + direction * maxDistance, p, q, r, t))
    {
        t *= maxDistance;
        return true;
    }
