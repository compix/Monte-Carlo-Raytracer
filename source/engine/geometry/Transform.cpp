...
#include <engine/event/TransformStaticStatusChangedEvent.h>
...
void Transform::onShowInEditor()
{
	...
	bool staticStatusChanged = ImGui::Checkbox("Static", &m_isStatic);
	...
	if (staticStatusChanged)
	{
		Event::transmit<TransformStaticStatusChangedEvent>(getOwner());
	}
}
...
void Transform::setLookDirection(const glm::vec3& direction, const glm::vec3& worldUp /*= glm::vec3(0.0f, 1.0f, 0.0f)*/)
{
	lookAt(getPosition() + direction, worldUp);
}
