...
uint8_t BBox::maxExtentIdx() const
{
    auto d = m_max - m_min;
    if (d.x > d.y && d.x > d.z)
        return 0;
...
uint8_t BBox::minExtentIdx() const
{
    auto d = m_max - m_min;
    if (d.x < d.y && d.x < d.z)
        return 0;
...
float BBox::maxExtent() const { return scale()[maxExtentIdx()]; }

float BBox::minExtent() const { return scale()[minExtentIdx()]; }

BBox BBox::toWorld(const glm::mat4& m) const
{
...
void BBox::transform(const glm::mat4& m) { *this = toWorld(m); }
