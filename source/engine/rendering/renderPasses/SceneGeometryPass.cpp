    ...
    : RenderPass("SceneRenderPass")
{
	...
	// Geometry Normals
	std::shared_ptr<Texture2D> geometryNormals = std::make_shared<Texture2D>();
	geometryNormals->create(Screen::getWidth(), Screen::getHeight(), GL_RGB8, GL_RGB, GL_UNSIGNED_BYTE);
	m_framebuffer->attachRenderTexture2D(geometryNormals, GL_COLOR_ATTACHMENT4);

	// Texture coordinates
	std::shared_ptr<Texture2D> uvs = std::make_shared<Texture2D>();
	uvs->create(Screen::getWidth(), Screen::getHeight(), GL_RGB8, GL_RGB, GL_UNSIGNED_BYTE);
	m_framebuffer->attachRenderTexture2D(uvs, GL_COLOR_ATTACHMENT5);
...
void SceneGeometryPass::update()
{
	...
	m_renderPipeline->put<GLuint>("GeometryNormalMap", getRenderTexture(GL_COLOR_ATTACHMENT4));
	m_renderPipeline->put<GLuint>("UVMap", getRenderTexture(GL_COLOR_ATTACHMENT5));
