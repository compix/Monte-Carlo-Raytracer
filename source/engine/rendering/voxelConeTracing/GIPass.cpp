...
void GIPass::update()
{
    ...
    m_finalLightPassShader->setFloat("u_occlusionDecay", VCT::GI_SETTINGS.occlusionDecay);
    m_finalLightPassShader->setFloat("u_ambientOcclusionFactor", VCT::GI_SETTINGS.ambientOcclusionFactor);
    ...
    m_finalLightPassShader->setFloat("u_traceStartOffset", VCT::GI_SETTINGS.traceStartOffset);
    m_finalLightPassShader->setFloat("u_stepFactor", VCT::GI_SETTINGS.stepFactor);

    int lightingMask = 0;
    lightingMask |= VCT::GI_SETTINGS.directLighting ? DIRECT_LIGHTING_BIT : 0;
    lightingMask |= VCT::GI_SETTINGS.indirectDiffuseLighting ? INDIRECT_DIFFUSE_LIGHTING_BIT : 0;
    lightingMask |= VCT::GI_SETTINGS.indirectSpecularLighting ? INDIRECT_SPECULAR_LIGHTING_BIT : 0;
    lightingMask |= VCT::GI_SETTINGS.ambientOcclusion ? AMBIENT_OCCLUSION_BIT : 0;

    m_finalLightPassShader->setInt("u_lightingMask", lightingMask);

    m_finalLightPassShader->setFloat("u_indirectDiffuseIntensity", VCT::GI_SETTINGS.indirectDiffuseIntensity);
    m_finalLightPassShader->setFloat("u_indirectSpecularIntensity", VCT::GI_SETTINGS.indirectSpecularIntensity);
    m_finalLightPassShader->setInt("u_visualizeMinLevelSelection", VCT::GI_SETTINGS.visualizeMinLevelSelection ? 1 : 0);
