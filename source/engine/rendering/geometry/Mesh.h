...
public:
    struct SubMesh
    {
		...
		BBox computeBBox() const { return Mesh::computeBBox(*this); }
		void scale(const glm::vec3& s) { Mesh::scale(*this, s); }
		void mapToUnitCube() { Mesh::mapToUnitCube(*this); }
    };
	...
	static BBox computeBBox(const SubMesh& subMesh);
    ...
    */
	...
	static void scale(SubMesh& subMesh, const glm::vec3& s);
	static void mapToUnitCube(SubMesh& subMesh);
private:
