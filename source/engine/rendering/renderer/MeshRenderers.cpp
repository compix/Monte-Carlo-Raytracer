...
std::shared_ptr<MeshRenderer> MeshRenderers::m_disk;
...
void MeshRenderers::init()
{
	...
	// Disk
	mesh = std::make_shared<Mesh>();
	mesh->load(GeometryGenerator::createDisk(0.5f, 128), VERTEX_POS | VERTEX_NORMAL | VERTEX_TANGENT | VERTEX_UV);
	m_disk = std::make_shared<MeshRenderer>(mesh);
	m_disk->addMaterial(material);
}
