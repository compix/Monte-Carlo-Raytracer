...
float MeshRenderer::computeSurfaceArea(uint32_t subMeshIdx)
{
	assert(subMeshIdx < m_mesh->getSubMeshes().size());

	auto transform = getComponent<Transform>();

	float area = 0.0f;
	auto& subMesh = m_mesh->getSubMeshes()[subMeshIdx];

	for (size_t i = 0; i < subMesh.indices.size() / 3; ++i)
	{
		const glm::vec3& p0 = transform->transformPointToWorld(subMesh.vertices[subMesh.indices[i * 3]]);
		const glm::vec3& p1 = transform->transformPointToWorld(subMesh.vertices[subMesh.indices[i * 3 + 1]]);
		const glm::vec3& p2 = transform->transformPointToWorld(subMesh.vertices[subMesh.indices[i * 3 + 2]]);

		float triangleArea = 0.5f * glm::length(glm::cross(p0 - p1, p0 - p2));
		area += triangleArea;
	}
