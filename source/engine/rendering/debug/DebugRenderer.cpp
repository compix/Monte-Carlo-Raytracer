...
std::shared_ptr<MeshRenderer> DebugRenderer::m_diskMeshRenderer;
...
void DebugRenderer::init()
{
	...
	m_diskMeshRenderer = MeshRenderers::disk();
...
void DebugRenderer::drawDisk(const glm::vec3& pos, float radius, const glm::vec4& color /*= glm::vec4(0.5f,0.5f,0.5f,1.0f)*/, float duration)
{
	bool depthTest = glIsEnabled(GL_DEPTH_TEST) == GL_TRUE;
	...
	m_diskMeshRenderer->render(m_debugShader.get());
	...
	if (duration > math::EPSILON)
		m_drawCommands.push_back(DrawCommand([=]() { drawDisk(pos, radius, color); }, duration));
}
